...
function init(){
    ...
    this.speed = 5;
    this.timer;
    this.pic_array = new Array("ycy1.jpg","ycy2.jpg","ycy3.jpg","ycy4.jpg","ycy5.jpg","ycy6.jpg","ycy7.jpg","ycy8.jpg");
    this.data_dic = "ycy/";
}
function generate(){       //生成白块
    ...
    var pkq = document.createElement('img');
    oDiv.children[index].appendChild(pkq);
    oDiv.children[index].children[0].src = 'pkq.jpg';
    oDiv.children[index].children[0].setAttribute('class', 'i');
    for (var i = 0; i < 4; i ++) {
        var ycy = document.createElement('img');
        if (i != index)
        {
            oDiv.children[i].appendChild(ycy);
            var jpg_src =data_dic + pic_array[Math.round(Math.random()*(pic_array.length-1))];
            oDiv.children[i].children[0].src = jpg_src;
        }
    }
...
function move(){     //整体移动
        ...
        if(len == 6){
            if(flag){
                for(var i = 0; i < 4 ; i ++){
                    if(main.children[len - 1].children[i].children[0].className == 'i'){
                        main.style.top = "-150px";
                        gameover();
